...
def create_defaultdict(init_vals):
    rev = defaultdict(int)
...
st_char_count = SegTree(
    N, e_factory=lambda: defaultdict(int), operator=operand_for_chars
)

for i, a in enumerate(A):
    st_is_ascending.update(i, [True, a, a])
    st_char_count.update(i, create_defaultdict([(a, 1)]))
...
for _ in range(Q):
    ...
    if t == "1":
        ...
        st_char_count.update(x, create_defaultdict([(c, 1)]))
    else:
        ...
        char_count = st_char_count.get_range(l, r)
        entire_char_count = st_char_count.get_range(0, N)
        is_started = False
        is_finished = False
        for s in "abcdefghijklmnopqrstuvwxyz":
            if not is_started:
                is_started = char_count[s] != 0
            elif not is_finished:
                is_finished = char_count[s] < entire_char_count[s]
            elif char_count[s] > 0:
                break
        else:
            print("Yes")
