# https://atcoder.jp/contests/tessoku-book/submissions/50036316
N = int(input())
...
a_max = max(A)
st = SegTree(a_max+1, 0) #st.tree[_]:対応する区間内で終わる狭義単調増加列の長さの最大値
for i in range(N):    
    left = 0
    right = A[i]
    m = max(st.get_range(left, right)+1, st.get_point(A[i]))
    st.update(A[i], m)
print(st.tree[1])